...
func main() {
	...
	// 这里我们创建了一个长度为 3 的 `string` 类型 slice（初始化为零值）
	s := make([]string, 3)
	fmt.Println(s)
	...
	// 其中一个事内建的 `append`，它返回一个包含了一个或者多个新值得 slice。
	// 注意由于 `append` 可能返回新的 slice，我们需要接受其返回值
	s = append(s, "d")
	s = append(s, "e", "f")
	fmt.Println("apd:", s)

	// Slice 也可以被 `copy`。这里我们创建一个空的和 `s` 有相同长度的 slice `c`，
	// 并且将 `s` 复制给 `c`
	c := make([]string, len(s))
	copy(c, s)
	fmt.Println("cpy:", c)

	// Slice 支持通过 `slice[low:high]` 语法进行切片操作
